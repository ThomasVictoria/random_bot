...
module.exports = async () => {
  ...
  let res = await request(
    `https://api.openweathermap.org/data/2.5/weather?id=${city.id}&units=metric&appid=${process.env.WEATHER}`
  ).then((res) => {
    return JSON.parse(res);
  });

  let country = await request(
    `https://restcountries.eu/rest/v2/alpha/${res.sys.country}`
  ).then((res) => {
    return JSON.parse(res);
  });
  ...
  return `*${res.name}, ${country.name}*\n${getWeatherIcon(
    res.weather[0].icon
  )}\n>${res.weather[0].description}, ${res.main.temp}°C, wind speed is ${
    res.wind.speed
  }km/h.`;
};
